# 部署指南（Cloudflare Pages + GitHub + 自定义 .cc 域名）
...
本地仓库 ──push──▶ GitHub 仓库 ──自动触发──▶ Cloudflare Pages 构建(npm run build)
                                            ...
                                            发布 dist/ 到全球 CDN
                                                    │
                                          绑定自定义域名 vlrlab.cc（HTTPS）
```

需要三个账号（都免费）：**GitHub**、**Cloudflare**、**域名注册商**（买 .cc 域名）。

---

## 第 1 步：注册 .cc 域名

在任一注册商购买你想要的 `.cc` 域名（如 `vlrlab.cc`、`vlrgroup.cc`）：

- 国际：Cloudflare Registrar（**最推荐**，见下方说明）、Namecheap、Porkbun、GoDaddy
- 国内：阿里云万网、腾讯云

> **强烈建议直接用 Cloudflare Registrar 买**（如果它支持 .cc）：域名和托管在同一家，DNS 自动配好，省去第 5 步的手动改 DNS。若在别处买，按第 5 步改 DNS 即可。
...
## 第 4 步：绑定自定义域名 vlrlab.cc

1. 在该 Pages 项目页 → **自定义域 Custom domains** → **设置自定义域**
2. 输入你的域名（如 `vlrlab.cc`，以及可选的 `www.vlrlab.cc`）→ 继续
3. 之后分两种情况：
...
1. 编辑 [`astro.config.mjs`](../astro.config.mjs)，把 `site` 改成你的域名：
   ```js
   site: 'https://vlrlab.cc',   // ← 换成你的真实域名（结尾无斜杠）
   ```
2. 提交推送：
   ```bash
